function nxd = calculateMirFeatures2( fileInformationArray )
%CALCULATEMIRFEATURES2 Summary of this function goes here
...
filePath = fileInformationArray{k,1};
% audio = miraudio(fileName);
frames  = mirframe(filePath);
...
for field = fieldnames(roughnessStruct)'
    
    if( strcmp('FileNames',(field{1})) ~= 1)
        featureVector(1,k) = roughnessStruct.(field{1})';
        k = k+1;
...
for field = fieldnames(irregularityStruct)'
    
    if( strcmp('FileNames',(field{1})) ~= 1)
        featureVector(1,k) = irregularityStruct.(field{1})';
        k = k+1;
