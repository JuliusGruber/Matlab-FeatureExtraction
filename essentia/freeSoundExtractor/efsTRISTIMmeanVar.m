...
% set t-SNE Paramters, this feature set has 6 features
no_dims = 2;
initial_dims = 6;
perplexity =5;

viewName = 'ESS_tri_mw_var';
...
%    %%%%%%% add features here %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%      
        ...
        triStimMean = cell2mat(YAMLStruct.sfx.tristimulus.mean);
        numFeatureValues = length( triStimMean);
        for b = 1 : numFeatureValues(1,1)
            featureVector(1,end+1)= triStimMean(1,b);
        end
     
        triStimVar = cell2mat(YAMLStruct.sfx.tristimulus.var);
        numFeatureValues = length( triStimVar);
        for b = 1 : numFeatureValues(1,1)
            featureVector(1,end+1)= triStimVar(1,b);
        end
